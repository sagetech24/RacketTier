        ...
        <div className="dashboard-v2-shell bg-[#131316] font-sans text-[#e4e1e6] selection:bg-[#c2c1ff] selection:text-[#282671]">
            ...
            <main className="mx-auto min-h-screen w-full max-w-3xl px-4 pb-32 pt-24">
                <div className="mb-4 flex items-start justify-between gap-3">
                    <div>
                        <h1 className="text-2xl font-extrabold tracking-tight">Queueing Sessions</h1>
                        <p className="text-sm text-[#c8c5d2]/80">Browse sessions, review status, and open your QM controls.</p>
                    </div>
                    <Link
                ...
                <section className="mb-4 rounded-xl border border-[#2a2a2d] bg-[#1b1b1e] p-4">
                    <div className="grid gap-3 grid-cols-4">
                        <input
                            value={q}
                            onChange={(e) => setQ(e.target.value)}
                            placeholder="Search by sport, QM, or session id"
                            className="col-span-4 rounded-lg border border-[#2a2a2d] bg-[#131316] px-3 py-2 text-sm"
                        />
                        <select
                            value={sort}
                            onChange={(e) => setSort(e.target.value)}
                            className="col-span-2 rounded-lg border border-[#2a2a2d] bg-[#131316] px-3 py-2 text-sm"
                        >
                    ...
                    <label className="flex items-center gap-2 mt-1 px-3 py-2 text-sm">
                        <input type="checkbox" checked={mineOnly} onChange={(e) => setMineOnly(e.target.checked)} />
                        My queueing sessions only
                    </label>
                </section>
                ...
                <div className="space-y-3">
                    {rows.map((row) => (
                        <article key={row.id} className="rounded-xl border border-[#2a2a2d] bg-[#1b1b1e] p-4">
                            <div className="mb-2 flex items-center justify-between gap-2">
                                <h2 className="flex items-center gap-2 text-base font-bold">
                                    <SportIcon icon={row.sport?.icon} className="text-[#4ce081]" />
                                    {row.sport?.name} Queue
                                </h2>
                                <span
                                    className={
                                        row.is_active
                            ...
                            <div className="mt-3 flex flex-wrap gap-2">
                                ...
                                <Link
                                    to={`/queueing-session/${row.id}/players`}
                                    className="rounded-lg border border-[#818184] px-3 py-1.5 text-xs font-semibold hover:border-[#4ce081]/60"
                                >
                                    Players
                                </Link>
                                <Link
                                    to={`/queueing-session/${row.id}/matches`}
                                    className="rounded-lg border border-[#818184] px-3 py-1.5 text-xs font-semibold hover:border-[#4ce081]/60"
                                >
                                    Matches
                                </Link>
